...
const isAllowedOrigin = (origin) => {
  ...
  if (process.env.ENVIRONMENT !== "production" && /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(origin)) return true;
  if (String(process.env.ALLOW_TUNNEL_ORIGINS || "false").toLowerCase() === "true" && (/\.ngrok-free\.app$/.test(origin) || /\.trycloudflare\.com$/.test(origin))) return true;
  return false;
};
