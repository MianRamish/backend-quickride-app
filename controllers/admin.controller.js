...
module.exports.approveCaptain = async (req, res) => {
  ...
  const required = ["license", "registration", "insurance", "governmentId", "profilePhoto", "vehiclePhoto"];
  const missing = required.filter((key) => reviews?.[key]?.status !== "approved");
...
module.exports.analyticsSummary = async (_req, res) => {
  ...
  const [totalPassengers, totalCaptains, totalRides, completed, cancelled, activeRides, onlineCaptains, pendingDrivers, openComplaints, openEmergencies] = await Promise.all([
    userModel.countDocuments({}), captainModel.countDocuments({}), rideModel.countDocuments({}),
    rideModel.countDocuments({ status: "completed" }), rideModel.countDocuments({ status: "cancelled" }),
    rideModel.countDocuments({ status: { $in: ["pending", "accepted", "arriving", "arrived", "ongoing"] } }),
    captainModel.countDocuments({ isOnline: true, socketId: { $ne: null } }),
    captainModel.countDocuments({ verificationStatus: "pending" }),
    require("../models/complaint.model").countDocuments({ status: { $in: ["open", "in_review"] } }),
    require("../models/emergency.model").countDocuments({ status: { $in: ["open", "in_review"] } }),
  ]);
  const revenueAgg = await rideModel.aggregate([{ $match: { status: "completed" } }, { $group: { _id: null, grossRideValue: { $sum: "$fare" }, platformCommission: { $sum: "$earnings.commissionAmount" }, cashCollectedValue: { $sum: { $cond: ["$cashCollected", "$fare", 0] } } } }]);
  const todayAgg = await rideModel.aggregate([{ $match: { status: "completed", completedAt: { $gte: today } } }, { $group: { _id: null, trips: { $sum: 1 }, fare: { $sum: "$fare" }, commission: { $sum: "$earnings.commissionAmount" } } }]);
  const owedAgg = await Settlement.aggregate([{ $match: { direction: "captain_owes_platform", status: { $in: ["owed", "partially_settled"] } } }, { $group: { _id: null, owed: { $sum: { $subtract: ["$amount", "$settledAmount"] } } } }]);
  return res.json({
    totalPassengers, totalCaptains, totalRides, completed, cancelled, activeRides, onlineCaptains, pendingDrivers, openComplaints, openEmergencies,
    revenue: revenueAgg?.[0]?.grossRideValue || 0, grossRideValue: revenueAgg?.[0]?.grossRideValue || 0, platformCommission: revenueAgg?.[0]?.platformCommission || 0, cashCollectedValue: revenueAgg?.[0]?.cashCollectedValue || 0,
    commissionOutstanding: owedAgg?.[0]?.owed || 0,
...
module.exports.updateCaptain = async (req, res) => {
  ...
  if (req.body.status !== undefined) {
    ...
    if (captain.status !== "active") {
      ...
      captain.currentOfferRide = null;
      captain.currentOfferExpiresAt = null;
    } else if (captain.availabilityStatus === "suspended") {
      captain.availabilityStatus = "offline";
    }
  }
  ...
  if (req.body.status !== undefined) {
    try {
      await require("../services/notification.service").notify({
        recipientType: "captain", recipient: captain._id, type: "account",
        title: captain.status === "suspended" ? "Driver account suspended" : "Driver account status updated",
        body: captain.status === "suspended" ? "Your driver account has been suspended. Contact QuickRide support for review." : `Your driver account is now ${captain.status}.`,
      });
    } catch (_) {}
  }
  await captain.populate("activeVehicle");
...
module.exports.updateRide = async (req, res) => {
  ...
  if (req.body.status !== undefined && req.body.status !== ride.status) {
    ...
    ride.statusTimeline = ride.statusTimeline || [];
    ride.statusTimeline.push({ status: req.body.status, at: new Date(), by: "admin", note: `Admin changed status from ${previousStatus}` });
    if (req.body.status === "cancelled") {
      ride.cancelledBy = "admin";
      ride.cancelReason = { code: "ADMIN_CANCELLED", text: req.body.cancelReasonText || "Cancelled by operations" };
...
module.exports.reviewCaptainDocument = async (req, res) => {
  const { id, docKey } = req.params;
  const { status, note = "" } = req.body;
  const allowedKeys = ["license", "registration", "insurance", "governmentId", "profilePhoto", "vehiclePhoto"];
  if (!allowedKeys.includes(docKey)) return res.status(400).json({ message: "Unknown document type" });
  if (!["pending", "approved", "rejected", "expired"].includes(status)) return res.status(400).json({ message: "Invalid review status" });
  const captain = await captainModel.findById(id);
  if (!captain) return res.status(404).json({ message: "Captain not found" });
  captain.documents.reviews[docKey] = { status, note: String(note).slice(0, 500), reviewedAt: new Date() };
  if (["rejected", "expired"].includes(status)) {
    captain.isApproved = false;
    captain.verificationStatus = status === "expired" ? "expired_documents" : "rejected";
    captain.verificationNote = note || `${docKey} ${status}`;
    captain.status = "inactive";
    captain.isOnline = false;
    captain.availabilityStatus = "offline";
  } else if (status === "approved" && captain.verificationStatus === "rejected") {
    captain.verificationStatus = "pending";
  }
  await captain.save();
  const notify = require("../services/notification.service");
  await notify.notify({ recipientType: "captain", recipient: captain._id, type: "verification", title: `Document ${status}`, body: `${docKey}: ${note || status}`, data: { docKey, status } });
  return res.json(captain);
};

module.exports.getPricing = async (_req, res) => {
  const config = await require("../services/operations.service").getConfig({ fresh: true });
  res.json(config);
};

module.exports.updatePricing = async (req, res) => {
  const AppConfig = require("../models/appConfig.model");
  const allowed = ["fares", "commissionRate", "cancellationFee", "cancellationFreeMinutes", "matchingRadiusKm", "rideOfferSeconds", "driverLocationStaleSeconds", "scheduledDispatchMinutes", "waitingFreeMinutes", "supportPhone"];
  const patch = {};
  allowed.forEach((key) => { if (req.body[key] !== undefined) patch[key] = req.body[key]; });
  // Card remains disabled until a real gateway integration is intentionally enabled in code.
  patch.cardPaymentsEnabled = false;
  const config = await AppConfig.findOneAndUpdate({ key: "operations" }, { $set: patch }, { new: true, upsert: true, runValidators: true });
  require("../services/operations.service").invalidateConfig();
  res.json(config);
};

module.exports.listSettlements = async (req, res) => {
  const Settlement = require("../models/settlement.model");
  const filter = req.query.status ? { status: req.query.status } : {};
  const items = await Settlement.find(filter).populate("captain", "fullname email phone earnings vehicle").populate("ride", "pickup destination fare completedAt").sort({ createdAt: -1 }).limit(500);
  res.json(items);
};

module.exports.settleCommission = async (req, res) => {
  try {
    const item = await require("../services/settlement.service").settle({ settlementId: req.params.id, amount: req.body.amount, reference: req.body.reference || "", adminNote: req.body.adminNote || "" });
    await item.populate("captain", "fullname email earnings");
    await item.populate("ride", "pickup destination fare");
    res.json(item);
  } catch (error) {
...
module.exports.listPromoCodes = async (_req, res) => {
  const Promo = require("../models/promoCode.model");
  res.json(await Promo.find({}).sort({ createdAt: -1 }).limit(300));
};

module.exports.createPromoCode = async (req, res) => {
  try {
    const Promo = require("../models/promoCode.model");
    const promo = await Promo.create({ ...req.body, code: String(req.body.code || "").trim().toUpperCase() });
    res.status(201).json(promo);
  } catch (error) {
    res.status(400).json({ message: error.code === 11000 ? "Promo code already exists" : error.message });
  }
};

module.exports.updatePromoCode = async (req, res) => {
  const Promo = require("../models/promoCode.model");
  const item = await Promo.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true, runValidators: true });
  if (!item) return res.status(404).json({ message: "Promo code not found" });
  res.json(item);
};

module.exports.liveOperations = async (_req, res) => {
  const config = await require("../services/operations.service").getConfig();
  const staleCutoff = new Date(Date.now() - Number(config.driverLocationStaleSeconds || 120) * 1000);
  const [drivers, rides] = await Promise.all([
    captainModel.find({ isOnline: true, socketId: { $ne: null } }).select("fullname vehicle location lastLocationAt availabilityStatus rating performanceScore").lean(),
    rideModel.find({ status: { $in: ["pending", "accepted", "arriving", "arrived", "ongoing"] } }).populate("user", "fullname phone").populate("captain", "fullname vehicle location lastLocationAt").sort({ createdAt: -1 }).limit(200).lean(),
  ]);
  res.json({
    drivers: drivers.map((driver) => ({ ...driver, locationFresh: Boolean(driver.lastLocationAt && new Date(driver.lastLocationAt) >= staleCutoff) })),
    rides,
